...
namespace ProviderImplementation.ProvidedTypes
    ...
    and ProvidedTypeDefinition(isTgt: bool, container:TypeContainer, className: string, getBaseType: (unit -> Type option), attrs: TypeAttributes, getEnumUnderlyingType, staticParams, staticParamsApply, backingDataSource, customAttributesData, nonNullable, hideObjectMethods) as this =
        ...
        override this.GetConstructorImpl(bindingFlags, _binder, _callConventions, types, _modifiers) = 
            let getParams (ci: ConstructorInfo) =
                ci.GetParameters() |> Array.map (fun x -> x.ParameterType)
            let xs = this.GetConstructors bindingFlags |> Array.filter (fun m -> m.Name = ".ctor" && (getParams m) = types)
...
namespace ProviderImplementation.ProvidedTypes
    ...
    type ProvidedTypesContext(referencedAssemblyPaths: string list, assemblyReplacementMap: (string*string) list, sourceAssemblies: Assembly list) as this =
        ...
        and tryConvConstructorRefToTgt (cons: ConstructorInfo) =
            ...
            let consT = declTyT.GetConstructor(bindAll, null, parameterTypesT, null)
            match consT with
...
namespace ProviderImplementation.ProvidedTypes
    ...
    type ILTypeBuilder(scoref, nsp: string uoption, nm: string, attrs: TypeAttributes) =
        ...
        member this.DefineTypeInitializer () = let mb = ILMethodBuilder(this.ILType, ".cctor", MethodAttributes.Static ||| MethodAttributes.SpecialName ||| MethodAttributes.RTSpecialName, ILType.Void, [| |]) in methods.Add mb; mb
